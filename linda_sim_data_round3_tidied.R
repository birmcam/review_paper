...
## now make fake patient-by-patient clusters
# choose subsets of each patient to be in groups A, B... etc.
# say these group sizes are exponentially distributed.

# say there are 25 different groups of patients - leave Z as a null
# allocate 10K patients to each group
#set.seed(0)
...
balanced <- TRUE
N_PATIENTS <- 10000
N_NULL <- 1000 # number of patients who aren't in any cluster
...
make_fake_data <- function(
  ...
  if(balanced==FALSE){
    ...
    patient_groups <- sample(LETTERS[1:K], N_PATIENTS, prob=rexp(K), replace=T)
  } else {
  ...
  for(disease_group_name in names(disease_groups)){
  	...
  	tmp <- rmvnorm(N, mean_vals, covmat)
